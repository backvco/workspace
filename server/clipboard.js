...
export function listClipboard(cfg, wsId) {
  ...
  for (const name of names) {
    ...
    items.push({ name, path: path.join(dir, name), size: st.size, mtime: st.mtimeMs, ext, type: EXT_TYPE[ext] || 'application/octet-stream' });
    totalBytes += st.size;
  }
...
// Delete items. `name` drops a single item; `all:true` wipes everything; else
// olderThanMs drops items older than (now - olderThanMs). Returns count + bytes freed.
export function pruneClipboard(cfg, wsId, { all = false, olderThanMs = null, name = null } = {}) {
  const dir = cfg.clipboardDir(wsId);
  ...
  for (const it of items) {
    const match = name != null ? it.name === name : all ? true : (cutoff != null && it.mtime < cutoff);
    if (!match) continue;
