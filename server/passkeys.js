...
// Revoke a user's outstanding code immediately (admin "undo"), no waiting for TTL.
export async function revokeEnrollCode(cfg, userId) {
  await q(cfg, 'DELETE FROM enroll_codes WHERE user_id = $1', [userId]);
  return { ok: true };
}
// Status of a user's outstanding code. We store only the HASH, so the plaintext
// can't be shown again — this reports whether an unexpired code exists and the
// whole minutes left, so the UI can show "active code, expires in N min".
export async function enrollCodeStatus(cfg, userId) {
  const rows = await q(cfg, 'SELECT created_at FROM enroll_codes WHERE user_id = $1', [userId]);
  const row = rows[0];
  if (!row) return { active: false };
  const leftMs = ENROLL_CODE_TTL_MS - (Date.now() - new Date(row.created_at).getTime());
  if (leftMs <= 0) return { active: false };
  return { active: true, expiresInMin: Math.ceil(leftMs / 60000) };
}
...
export async function redeemEnrollCode(cfg, userId, code) {
  ...
  return validCode(cfg, rows[0], code);
}
// Non-destructive validity check. Used at LOGIN to gate the enrollment-code path
// (let a password alone issue a session under passkey/2FA policies so a brand-new
// device with no passkey can be enrolled). The code is actually consumed later at
// register-verify via redeemEnrollCode, so it stays single-use end-to-end.
export async function peekEnrollCode(cfg, userId, code) {
  if (!canonCode(code)) return false;
  const rows = await q(cfg, 'SELECT code_hash, created_at FROM enroll_codes WHERE user_id = $1', [userId]);
  return validCode(cfg, rows[0], code);
}
function validCode(cfg, row, code) {
  if (!row) return false;
