...
# and runs from server/, so its deps must be installed there too. Don't let a build
# failure here (e.g. node-pty needing macOS Xcode tools) abort the whole installer.
say "Installing server dependencies..."
( cd server && npm install ) || true
if [ ! -d server/node_modules/express ]; then
  echo
  echo "ERROR: server dependencies did not install. The most common cause is the"
  echo "native module 'node-pty' failing to build (needs a C/C++ toolchain)."
  echo "  - macOS: install Xcode Command Line Tools, then retry:"
  echo "      xcode-select --install   # click Install, wait for it to finish"
  echo "  - then:  (cd '$(pwd)/server' && npm install)"
  echo "Re-run ./install.sh (or 'node bin/setup.mjs') once that succeeds."
  exit 1
fi
