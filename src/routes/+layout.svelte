  ...
  <header class="flex items-center h-9 px-3 bg-card border-b border-line text-sm">
    ...
    <div class="ml-auto flex items-center gap-4">
      ...
      {#if stats}
        <div class="relative">
          <button
            class="flex items-center gap-4 text-xs text-muted font-mono hover:text-content"
            title="Per-core CPU + memory"
            onclick={(e) => { e.stopPropagation(); statsOpen = !statsOpen; }}
          >
            <span>CPU <span class={col(stats.cpu)}>{stats.cpu}%</span></span>
            <span>MEM <span class={col(stats.mem.percent)}>{stats.mem.percent}%</span>
              <span class="opacity-50">{stats.mem.usedGb}/{stats.mem.totalGb}G</span></span>
          </button>
          {#if statsOpen}
      ...
      <button class="text-muted hover:text-content leading-none" title="Settings" aria-label="Settings"
        onclick={() => openTool({ toolId: 'settings', title: 'Settings', singleton: true })}>
        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
